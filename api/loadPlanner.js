...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 2) Bootstrap if empty (first run)
    if (!groups || groups.length === 0) {
      const defaults = [
        { name: "Throttle", position: 0 },
        { name: "Neutral", position: 1 },
        { name: "Send It", position: 2 }
      ];

      const ins = await supabase.from("groups").insert(defaults).select("*").order("position");
      if (ins.error) {
        console.error("Supabase groups bootstrap insert error:", ins.error);
        return res.status(500).json({ error: "Failed to bootstrap groups", details: ins.error.message });
      }
      groups = ins.data || [];
    }
    ...
    // 4) Normalize payload for client
    ...
    return res.status(200).json({ groups: normalizedGroups, projects: normalizedProjects });
  } catch (err) {
