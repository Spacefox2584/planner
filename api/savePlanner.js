...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!Array.isArray(groups) || !Array.isArray(projects)) {
      return res.status(400).json({ error: "Invalid payload: 'groups' and 'projects' must be arrays" });
    }
    ...
    const gRecords = groups.map((g, idx) => {
      ...
      if (isUuid(g.id)) record.id = g.id; // allow explicit UUID
      return record;
    });
    ...
    const pRecords = projects.map((p) => {
      ...
      if (isUuid(p.id)) record.id = p.id; // allow explicit UUID
      return record;
    });

    const supabase = getClient();

    // Replace snapshot (FK-safe ordering)
    ...
    if (delProjects.error) {
      ...
      return res.status(500).json({ error: "Delete projects failed", details: delProjects.error.message });
    }

    const delGroups = await supabase.from("groups").delete().neq("id", null);
    if (delGroups.error) {
      console.error("Supabase delete groups error:", delGroups.error);
      return res.status(500).json({ error: "Delete groups failed", details: delGroups.error.message });
    }

    // Insert groups
    let insertedGroups = [];
    if (gRecords.length > 0) {
      const gi = await supabase.from("groups").insert(gRecords).select("id,name,position").order("position");
      if (gi.error) {
        console.error("Supabase insert groups error:", gi.error);
        return res.status(500).json({ error: "Insert groups failed", details: gi.error.message });
      }
      insertedGroups = gi.data || [];
    }

    // Map old group ids to inserted ones if the client sent no ids (edge case)
    const haveClientGroupIds = gRecords.every(gr => !!gr.id);
    let groupIdSet = new Set(insertedGroups.map(g => g.id));
    if (!haveClientGroupIds && groupIdSet.size) {
      // If client didn't send ids, the project
